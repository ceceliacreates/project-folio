    ...
    >
      ...
      <v-combobox
        ...
        <v-btn class="white--text"
          color="cyan"
          raised data-cy="submit" :disabled="!valid" @click="submit">Submit</v-btn>
      </v-col>
    </v-row>
  </v-container>
</template>
...
export default {
  ...
  "NoSQL"
      ...
      tagRules: [
          (v) => v.length > 0 || "At least one tag required"
      ],
      urlRules: [
          (v) => !!v || "Project URL is required",
          (v) => /[(http(s)?):\/\/(www\.)?a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/.test(v) || "Must be a valid URL"
      ],
      descriptionRules: [
          (v) => !!v || "Project Description is required",
          (v) => v.length >= 10 || "Description must be greater than 10 characters"
      ]
    };
  },
  methods: {
